...
function addField(id){
    ...
    nodesItems.value[id].data.fields.push({
        id: lastOne+1,
...
<template>
    ...
    >
        <Panel v-show="react.sidebar"
               position="top-left"
               class="h-screen bg-gray-200 border-r border-gray-300 flex flex-col justify-start overflow-y-scroll" style="width: 35vw; margin: 0;">
            <div class="flex justify-between gap-4 bg-white border-b border-gray-300 p-4 shadow-sm">
            ...
            <div v-for="(item, key) in nodesItems" >
                <div v-if="!item.isEdge || item.isEdge === '0'">
                    <div class="flex justify-between gap-4 border-b border-gray-300 py-3 w-full px-4 bg-white"  :style="'border-left: '+ item.data.color +' 8px solid'">
                        <div class="w-full cursor-move">
                            ...
                            <div v-show="item.data.showTableNameEdit && item.data.showTableNameEdit !== '0'">
                                <input type="text" :ref="el => { tables[item.data.table] = el }" v-model="nodesItems[key].data.table" @keypress.enter.prevent="item.data.showTableNameEdit = false" placeholder="Table Name" class="block w-full h-10 border border-gray-200 bg-gray-400/10 placeholder-gray-500  transition duration-75 rounded-lg focus:bg-white focus:placeholder-gray-400 focus:border-primary-500 focus:ring-1 focus:ring-inset focus:ring-primary-500 dark:bg-gray-700 dark:text-gray-200 dark:placeholder-gray-400" />
                            </div>
                        </div>
                        <div class="flex justify-start gap-4">
                            <div class="flex flex-col justify-center items-center" v-tooltip="__('edit_table_name')" @click.prevent="item.data.showTableNameEdit = !item.data.showTableNameEdit">
                    ...
                    <div class="bg-white border-b border-gray-300 p-2 flex flex-col gap-4" v-show="item.data.show && item.data.show !== '0'">
                        <VueDraggableNext
                        ...
                        >
                            <div class="grid grid-cols-12 gap-2" v-for="(field, index) in item.data.fields">
                                ...
                                <div class="col-span-4">
                                    <select v-model="nodesItems[key].data.fields[index].type" class="block w-full h-10 border border-gray-200 bg-gray-400/10 placeholder-gray-500 transition duration-75 rounded-lg focus:bg-white focus:placeholder-gray-400 focus:border-primary-500 focus:ring-1 focus:ring-inset focus:ring-primary-500 dark:bg-gray-700 dark:text-gray-200 dark:placeholder-gray-400">
                                ...
                                <div class="flex flex-col justify-center items-center">
                                    <button class="font-bold" @click.prevent="nodesItems[key].data.fields[index].nullable = !nodesItems[key].data.fields[index].nullable" :class="{
                                        'text-primary-500' : nodesItems[key].data.fields[index].nullable
                                    }">N</button>
                                </div>
                                <div class="flex flex-col justify-center items-center">
                                    <button class="flex flex-col justify-center items-center" @click.prevent="nodesItems[key].data.fields[index].key_type_show = !nodesItems[key].data.fields[index].key_type_show; nodesItems[key].data.fields[index].more_option_show = false">
                                ...
                                <div class="flex flex-col justify-center items-center">
                                    <button @click.prevent="nodesItems[key].data.fields[index].more_option_show = !nodesItems[key].data.fields[index].more_option_show; nodesItems[key].data.fields[index].key_type_show = false">
                                        <i class="bx bx-dots-vertical" ></i>
                                    </button>
                                </div>
                                <div class="col-span-12" style="margin: 0 -8px 0 -8px !important;">
                                    <div class="p-4 flex flex-col gap-4 bg-gray-100" v-if="nodesItems[key].data.fields[index].key_type_show">
                                        <div>
                                        ...
                                        <div class="flex justify-between gap-2 hover:bg-primary-500 px-4 py-1 hover:text-white rounded-lg cursor-pointer" @click.prevent="nodesItems[key].data.fields[index].index_type = 'unique';  nodesItems[key].data.fields[index].linked_to_table = ''; nodesItems[key].data.fields[index].linked_to_field = ''">
                                            <div class="flex justify-start gap-2 w-full">
                                        ...
                                        <div class="flex justify-between gap-2 hover:bg-primary-500 hover:text-white px-4 py-1 rounded-lg cursor-pointer" @click.prevent="nodesItems[key].data.fields[index].index_type = null;  nodesItems[key].data.fields[index].linked_to_table = ''; nodesItems[key].data.fields[index].linked_to_field = ''">
                                            <div class="flex justify-start gap-2 w-full">
                                                <div  class="flex flex-col justify-center items-center w-4">
                                    ...
                                    <div class="p-4 flex flex-col gap-4 bg-gray-100" v-if="nodesItems[key].data.fields[index].more_option_show">
                                        <div>
                                        ...
                                        <div>
                                            <label class="text-gray-400 text-sm">{{__('actions')}}</label>
                                            <div>
                                                <button class="border rounded-lg px-4 py-1 my-2 hover:bg-red-500 hover:text-white" @click.prevent="removeCol(index, key)">
                                                    {{ __('delete_column') }}
                                                </button>
                                            </div>
                                        </div>
                        ...
                        <div class="flex justify-start gap-2">
                            <button @click.prevent="addField(key, item.data.fields[item.data.fields.length-1].position, item.data.fields[item.data.fields.length-1].id)" class="filament-button inline-flex items-center justify-center py-1 gap-1 font-medium rounded-lg border transition-colors focus:outline-none focus:ring-offset-2 focus:ring-2 focus:ring-inset dark:focus:ring-offset-0 min-h-[2.25rem] px-4 text-sm shadow-sm focus:ring-white filament-page-button-action bg-primary-600 hover:bg-primary-500 focus:bg-primary-700 focus:ring-offset-primary-700 text-white border-transparent">
                                {{ __('add_col') }}
                            </button>
                            <button @click.prevent="removeNode(item.id)" class="filament-button inline-flex items-center justify-center py-1 gap-1 font-medium rounded-lg border transition-colors focus:outline-none focus:ring-offset-2 focus:ring-2 focus:ring-inset dark:focus:ring-offset-0 min-h-[2.25rem] px-4 text-sm shadow-sm focus:ring-white filament-page-button-action bg-danger-600 hover:bg-danger-500 focus:bg-primary-700 focus:ring-offset-danger-700 text-white border-transparent">
                                {{__('delete_table') }}
                            </button>
                            <button @click.prevent="goToForm(item.data.table)" class="filament-button inline-flex items-center justify-center py-1 gap-1 font-medium rounded-lg border transition-colors focus:outline-none focus:ring-offset-2 focus:ring-2 focus:ring-inset dark:focus:ring-offset-0 min-h-[2.25rem] px-4 text-sm shadow-sm focus:ring-white filament-page-button-action bg-primary-600 hover:bg-primary-500 focus:bg-primary-700 focus:ring-offset-primary-700 text-white border-transparent">
                                {{__('edit_from') }}
                            </button>
                        </div>
                    </div>
            ...
            <div class="min-w-32 h-full bg-white rounded-md overflow-hidden shadow-sm">
                ...
                <div class="text-xs">
                    <div class="border-b p-1 flex justify-around hover:text-purple-400 hover:bg-gray-100 gap-6" v-for="(item, key) in data.fields">
                        <Handle :connectable="false" :id="item.name+'_right_'+data.table" type="source" :position="Position.Right" :style="{
                            backgroundColor: data.color,
                            top: (key === 0 ? 40 : key === 1 ? 40+25 : 40+(25*key)).toString()+'px',
                        }" />
                        <div class="font-bold w-full">
                            <div class="flex justify-start gap-2" @click.prevent="tables[data.table+'_'+key].focus()">
                        ...
                        <Handle  :connectable="false" :id="item.name+'_left_'+data.table" type="target" :position="Position.Left" :style="{
                            backgroundColor: data.color,
                            top: (key === 0 ? 40 : key === 1 ? 40+25 : 40+(25*key)).toString()+'px',
                        }" />
                    </div>
